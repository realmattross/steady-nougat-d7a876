...
Use web search for current info: news, weather, sports, research. Be concise.`;
    ...
    if(last.startsWith("SEND_EMAIL|")){
      ...
      await saveHist([...hist,{u:txt.substring(0,400),a:reply.substring(0,800)}]);
      await fetch("https://steady-nougat-d7a876.netlify.app/push-hud",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({question:txt.slice(0,60),answer:clean(reply).slice(0,1500)})}).catch(()=>{});
